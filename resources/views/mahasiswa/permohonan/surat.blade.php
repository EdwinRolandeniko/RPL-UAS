...
<h3 class="center">SURAT PERMOHONAN</h3>
@foreach($kd as $data)
<h3 class="center"> {{ $data->kode_surat }}/A/FTI/{{ $permohonan->tgl_kirim->format('Y') }}</h3>                   
@endforeach


<p>{{ $permohonan->id_user}}</p>
<p>{{ $permohonan->nama_mitra}}</p>
<p>{{ $permohonan->alamat_mitra}}</p>
<p>{{ $permohonan->tgl_kirim->format('d-m-Y')}}</p>
